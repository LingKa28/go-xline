...
func newFixedRetryConfig(delay time.Duration, count uint) *retryConfig {
	...
	return &retryConfig{
		backoff: &backoffConfig{fixed: &struct{}{}},
		delay:   delay,
		count:   count,
	}
}
...
func newExponentialRetryConfig(delay time.Duration, maxDelay time.Duration, count uint) *retryConfig {
	...
	return &retryConfig{
		backoff: &backoffConfig{
			exponential: &struct{ maxDelay time.Duration }{maxDelay: maxDelay},
		},
		delay: delay,
		count: count,
	}
...
func (b *backoff) nextDelay() *time.Duration {
	...
	b.count--
	cur := b.curDelay
	if b.config.backoff.exponential != nil {
		b.curDelay *= 2
		if b.curDelay > b.config.backoff.exponential.maxDelay {
			b.curDelay = b.config.backoff.exponential.maxDelay
		}
	}
	return &cur
}
